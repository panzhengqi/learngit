...
can_manager::can_manager()
{
    ...
    p_can_send_manager      = new can_send_manager();
...
//        ioctl(can_fd, SIOCGIFNAME, &ifr);
//        qDebug("Received a CAN frame from interface id %x\n", (p_can_frame->can_id));
//        qDebug("SIZE %dBytes, DATA ", p_can_frame->can_dlc);
//        for (i = 0; i < p_can_frame->can_dlc; i++) {
//            printf("%#x ", p_can_frame->data[i]);
//        }
//        printf("\n");
//        can_receive_frame_count++;
//        qDebug("CAN receive frame count: %lld", can_receive_frame_count);
