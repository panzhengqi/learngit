...
//    connect((QObject *)check_recv_timer, SIGNAL(timeout()), this, SLOT(check_can_bus_recv()));
    p_can_recv                = new can_recv(can_fd);
...
//    connect((QObject *)p_can_send_manager, SIGNAL(send_can_frame(can_frame*)), this, SLOT(can_frame_send(can_frame*)));
    ...
    p_can_recv_manager_thread = new QThread();
    connect((QObject *)p_can_recv, SIGNAL(transmit_can_recv_msg(can_frame *)), p_can_recv_manager, SLOT(accept_can_frame(can_frame*)));
    ...
    p_can_recv->moveToThread(p_can_recv_thread);
    p_can_recv_manager->moveToThread(p_can_recv_manager_thread);
    ...
    p_can_recv_manager_thread->start();
...
//    }
    ...
    if(p_can_recv_manager_thread){
        p_can_recv_manager_thread->terminate();
        p_can_recv_manager_thread->wait();
        delete p_can_recv_manager_thread;
        p_can_recv_manager_thread = NULL;
    }
